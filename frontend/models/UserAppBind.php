...
class UserAppBind extends FActiveRecord
{
    ...
    public function potatoLists($p)
    {
        ...
        $res = self::find()->where(['user_id'=>$userId,'type'=>UserAppBind::APP_TYPE_POTATO])->offset($offset)->limit($limit)->orderBy('sort asc ,id asc')->all();
    ...
    public function telegramLists($p)
    {
        ...
        $limit =  self::PAGE_NUM;
        $offset = $p == 0 ? 0: self::PAGE_NUM*$p;
        $res = self::find()->where(['user_id'=>$userId,'type'=>UserAppBind::APP_TYPE_TELEGRAM])->offset($offset)->limit($limit)->orderBy('sort asc ,id asc')->all();
        return  $this->jsonResponse($res,'操作成功',0, ErrCode::SUCCESS);
