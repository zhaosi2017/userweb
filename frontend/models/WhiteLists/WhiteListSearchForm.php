    ...
    public function newLists($p)
    {
        ...
        if(!empty($res))
        {
            foreach ($res as $k => $v)
            {
                ...
                $_user = User::find()->select(['nickname','account','header_img'])->where(['id'=>$v['white_uid']])->one();
                if(  !empty($_user)) {
                    $data['lists'][$k]['account'] = $_user->account;
                    $data['lists'][$k]['remark'] = isset($_friends['remark']) && $_friends['remark'] ? $_friends['remark'] : $_user['nickname'];
                    $data['lists'][$k]['header_url'] = $_user['header_img'] ?  Yii::$app->params['frontendBaseDomain'].$_user['header_img'] :'';
                }
            }
        ...
        $_tmp = User::find()->select('whitelist_switch')->where(['id'=>$userId])->one();
        $data['whitelist_switch'] = isset($_tmp['whitelist_switch']) ? $_tmp['whitelist_switch'] : 0;
        return  $this->jsonResponse($data,'操作成功',0, ErrCode::SUCCESS);
